...
In Lektion [Teiler und Vielfache](teiler_vielfache.qmd#sec-ggT) haben wir den größten gemeinsamen Teiler zweier ganzer Zahlen iterativ berechnet mit folgender Funktion:
...
Benannt ist die Folge nach dem italienischen Mathematiker Leonardo Fibonacci, den wir bereits in der Lektion [Zahlenbereiche und Zahlensysteme](zahlen.qmd#sec-dezimal) kennengelernt haben.
...
Wir können aber erkennen, dass sich die Funktion bei der Abarbeitung sehr oft mit kleineren Argumenten selbst aufruft.
Es kann gezeigt werden, dass dies zu einem exponentiellen Wachstum der Arbeitsschritte für größere Eingaben führt; das kann man dann in mathematischer Schreibweise wie folgt ausdrücken:

$$
\mathcal{O}(\varphi^n),
$$

was wir wie folgt lesen können: \
die Anzahl der Schritte zur Abarbeitung des Algorithmus liegt in der Größenordung von $\varphi^n$,
wobei der griechische Buchstabe $\varphi$ (gesprochen als phi) einen konstanten Faktor von ungefähr 1,618 angibt.
Da $n$ im Exponent dieser Formel steht, sprechen wir von einem exponentiellen Wachstum.

Schon bei einem Eingabewert von 35 müssen wir mit $1,618^{35} \approx 2.1e7$, also mehr als als 20 Millionen Aufrufen rechnen.
...
Die Bearbeitungszeit beträgt knapp eine Sekunde.
...
Wir messen die Laufzeit der neuen Version ebenfalls mit dem Eingabewert 35:

```{python}
%time fib_mem(35)
```

Die Laufzeit beträgt jetzt rund 5 Mikrosekunden ($1\mu s=10^{-6}s$).
...
Die Laufzeit beträgt hier 2 $\mu s$, die iterative Version ist also ungefähr doppelt so schnell wie die optimierte rekursive Version.
...
Du kannst dir den Quellcode dafür unter `arithmetik/_scripts/hanoi_solve.py` ansehen und dort auch die Anzahl der Scheiben ändern und dann mit `manim -ql hanoi_solve.py` die Animation neu erstellen.
...
Dazu nehmen wir an, dass unsere intuitiv gefundene Formel aus @eq-zuege korrekt ist und werden dies mittels *mathematischer Induktion* beweisen.
...
Der erste Fall für $n_0=0$ (*Induktionsanfang*) ist trivial, da $H_0 = 2^0-1=1-1=0$.
...
Da die Behauptung für ein beliebig ausgewähltes $n_0$ und für $n-1$ gilt, gilt sie nach den Regeln der vollständigen Induktion für jedes beliebige $n$.
