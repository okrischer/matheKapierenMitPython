...
Der Rest ist also die Differenz zwischen dem Dividenden $n$ und dem größten Vielfachen des Divisors $m$: $\ r=n-m\cdot q$.\
...
Wir werden uns in späteren Kapiteln ausführlich mit Funktionen beschäftigen;
...
Unsere Funktion `mod` benutzt den Python Operator `//` für die ganzzahlige Division.
Wir wissen aber nicht, wie dieser Operator in Python implementiert ist, d.h. wie hier das Ergebnis berechnet wird.
Deshalb wollen wir eine eigene Funktion schreiben, die diese Berechnung im einfachsten Fall veranschaulicht:
wir berechnen den Quotienten und den Rest einer ganzzahligen Division mittels wiederholter Subtraktion, und nennen die Funktion `quotrem`.
...
assert quotrem(2,4) == (0, 2)
assert quotrem(4,4) == (1, 0)
```
1. Initialisiert den Quotienten `q` mit 0
2. Beginn einer Schleife, die solange ausgeführt wird, wie `n` größer oder gleich `m` ist
3. Ziehe `m` einmal von `n` ab
4. Erhöhe den Quotienten `q` um eins
5. Gib das Ergebis von Quotient und Rest der Divsison als Tupel zurück.

Die Funktion `quotrem` macht nichts anderes als zu abzuzählen, wie oft der Divisor `m` in den Dividend `n` passt.
Sobald `n` kleiner als `m` ist, passt `m` kein weiteres mal in `n`, und `n` ist der Rest der Division.
Die Funktion `quotrem` ist dabei nicht sonderlich effizient, da sie $n \div m$ Subtraktionen durchführen muss.
...
::: {.callout-tip}
## Größter gemeinsamer Teiler für 3528 und 3780
...
2. Bei einer Differenz von 0 ist man fertig und die kleinere Länge ist das Ergebnis.
...
Wir machen uns diesen Umstand zunutze und definieren eine neue Funktion in Python für die Berechnung des *kgV* für drei Zahlen:
...
Falls du Python und Jupyter noch nicht installiert hast, dann sieh in Lektion [Rechenregeln](rechenregeln.qmd#sec-ubung) nach und richte deine Arbeitsumgebung ein.

Um mit den Jupyter-Notebooks dieses Tutorials arbeiten zu können, musst du sie dir zunächst herunterladen.
Besuche dazu die Seite [matheKapierenMitPython](https://github.com/okrischer/matheKapierenMitPython) in deinem Bowser, klicke auf den grünen Button `<Code>` und wähle die unterste Option `<Download ZIP>`, um dir alle Inhalte dieses Verzeichnisses auf deinen Rechner zu laden.
Entpacke das heruntergeladene Archiv `matheKapierenMitPython-main.zip`, indem du in deinem File-Explorer doppelt auf den Dateinammen klickst.

Wechsle dann in deiner Eingabeaufforderung (Prompt) oder in deinem Terminalfenster in das Verzeichnis `matheKapierenMitPython-main` und aktiviere deine Arbeitsumgebung mit `conda activate matheKapieren`.

Jetzt kannst du mit `jupyter lab` *Jupyter* starten, und es sollte sich ein neues Browser-Fenster öffnen.
Öffne dort im *File Browser* (der Spalte ganz links im Browser-Fenster) das Verzeichnis `arithmetik`, indem du einen Doppelclick auf den Verzeichnisnamen ausführst, und öffne dann das Verzeichnis `_notebooks`.

Hier findest du eine Reihe von Jupyter Notebooks;
um die Inhalte dieser Lektion einzuüben, bearbeite die Notebooks `euler1.ipynb` und `exercism_raindrops_ipynb`, indem du wieder doppelt auf den jeweiligen Namen klickst.
Folge dann den Anweisungen im jeweiligen Notebook.