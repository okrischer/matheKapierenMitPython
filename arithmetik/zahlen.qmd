...
::: {.callout-important}
## Schnittmenge
Die Schnittmenge $A\cap B$ zweier Mengen $A$ und $B$ besteht aus denjenigen Elementen, die sowohl in $A$ als auch in $B$, also gleichzeitig in beiden Mengen $A, B$ enthalten sind:
$$
...
Hier haben wir den Modulo Operator `%` verwendet, der den Rest einer [ganzzahligen Division](teiler_vielfache.qmd#sec-division) ausgibt, um zu testen, ob `z` eine ungerade Zahl ist.
...
Tragischerweise hat ausgerechnet *Hippasos*, ein Schüler des *Pythagoras*, mit Hilfe dessen Satzes bewiesen, dass $\sqrt{2}$ keine rationale Zahl sein kann.
...
Die Zahlen in $\mathbb{R}$ können in einer *Zahlengeraden* veranschaulicht werden, die verdeutlicht, dass die Menge der reelen Zahlen über die Vergleiche $<$ und $>$ eine *lineare Ordnung* bildet.
...
Wir haben unser *dictionary* `rom2dec` mit den Werten aus @tbl-ziffern initialisiert.
...
def roman_to_decimal(roman):                                # <1>
  ...
  for i in range(len(roman)):                               # <3>
    if i > 0 and rom2dec[roman[i]] > rom2dec[roman[i-1]]:   # <4>
      res += rom2dec[roman[i]] - 2 * rom2dec[roman[i-1]]    # <5>
    else:
      res += rom2dec[roman[i]]                              # <6>
  return res                                                # <7>
...
2. Teilen des Eingabestrings in zwei Hälften, den Teil vor dem Dezimalpunkt und den Teil nach dem Punkt (Python verwendet zur Darstellung von Dezimalzahlen den im englischen Sprachraum üblichen Dezimalpunkt anstelle eines Kommas).
...
4. Wir durchlaufen die Zeichen im ersten Teilstring (die Ziffern vor dem Punkt) und berechnen den Wert der jeweiligen Ziffer nach der Formel von @eq-dualzahl.
