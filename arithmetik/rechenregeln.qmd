...
So werden zum Beispiel die vier Grundrechenarten Addition, Subtraktion, Multiplikation und Division durch die Operatoren $+, \ -, \ \cdot, \ \div$ ausgedrückt, während das Gleichheitszeichen ($=$) das Ergebnis der Berechnung widerspiegelt.
...
# Die nächste Zeile gibt den Wert von c aus
c
```
...
Wir werden im nächsten Kapitel ??? sehen, wie wir diese Beschränkung umgehen können, indem wir eine zusätzliche Bibliothek (engl. *library*) installieren.
...
Produkte von Summen und Differenzen werden berechnet, indem man ihre Glieder *ausmultipliziert*.
...
Mit der Mathematik verhält es sich nicht anders als mit dem Fußballspielen oder dem Klavierspielen.
Wenn wir Fußballspielen lernen wollen, dann reicht es nicht, ein Fußballspiel anzuschauen; wenn wir Klavierspielen lernen wollen, dann reicht es nicht, ein Klavierkonzert anzuhören.
Wir müssen es selbst tun, es wiederholt üben, um die notwendige Geschicklichkeit zu erlangen.
Genauso können wir Mathematik nicht lernen, indem wir ein Tuturial (wie dieses hier) oder ein Lehrbuch über Mathematik lesen.

Wir müssen auch Mathematik üben, selbst Berechnungen durchführen und dabei die gültigen Regeln anwenden, um mathematische Geschicklichkeit zu erlangen.
Das ist weder schwer noch besonders aufwändig; wir müssen es einfach nur tun.

Deshalb enthält jedes Kapitel einen abschließenden Abschnitt **Übungen**.
Ich werde dir ein paar Aufgaben vorgeben, die du mit etwas Nachdenken sicher lösen kannst.
Wenn du bei einer Aufgabe dennoch Schwierigkeiten hast, schau dir den entsprechenden Lösungshinweis an, und vergleiche dann deine Lösung mit meinem Lösungsvorschlag.
